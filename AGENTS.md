...
- `@crp/extension` 代码交付后必须执行 `pnpm --filter @crp/extension build` 重新打包，否则 `dist/` 仍是旧产物，reload 插件不会生效。
...
凡涉及新增或修改 AI 基建时，必须先阅读 `docs/ai-maintenance.md`，包括但不限于 rules、skills、commands、permissions、Cursor / Codex 专属配置。
