...
> 2026-06-29 刷新说明：这是历史执行计划，不是最新任务清单。当前扩展行为、构建方式、fallback 和测试说明
> 以 `docs/extension.md`、`docs/api-notes.md`、`docs/development.md` 为准。
...
**环境约定（必读）：** 见仓库根 `AGENTS.md`。所有 Shell 命令必须带 `required_permissions: ["all"]`；用 **pnpm**；PowerShell 下不要用 `tail/head/grep`。
...
- 函数：`getRatingChanges(contestId)`、`getStandings(contestId)`、`getUserInfos(handles: string[])`。当前实现中
  `user.info` 按 300 handles 分批、最多 8 批并发，并缓存 6 小时。
...
- 不改动 `@crp/core` 公共接口；如需新增 core 能力（如升档所需 delta API）另开任务。
