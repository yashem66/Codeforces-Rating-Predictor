...
- 日期：2026-06-28
- 状态：已与用户确认设计方向，待用户复审本文档后转入实现计划（writing-plans）
...
- 通胀修正（327 轮后版本，两步）：
  ...
  2. 取按 rating 降序的前 `s = min(n, round(4*sqrt(n)))` 人，`sumTop = Σ这 s 人的 d_i`，
...
- 论坛 2026-04 反馈 `contest.standings` 被限制；**实测**：`contest.standings?contestId=566`
  ...
  即官方恢复了**受限的匿名公开模式**（不可加 `from/count/handles/showUnofficial`/鉴权等参数）。
...
│  │  │  ├─ performance.ts      # performance()/deltaToRankUp()（接口预留，本期可留桩）
│  │  │  ├─ types.ts
│  │  │  └─ index.ts
...
- `computeRatingChanges(contestants: Contestant[]): RatingChange[]`：
  ...
  5. 修正②：`s = min(n, round(4*sqrt(n)))`，按 rating 降序取前 s 人 `sumTop`，
...
- 预留（本期留桩/最小实现）：`performance(contestant, others)`（delta 为 0 时的 rating）、
  `deltaToRankUp(...)`（升档所需 delta）——供未来直播列使用。
...
- 指标：每场 & 汇总的 delta 精确命中率、`|误差|` 均值/中位/最大、误差直方图、最差 mismatch 清单。
  输出 JSON + 可读摘要。
- 命令：
  - `crp fetch --sample` / `crp fetch --range 2022-01:..`：构建数据集缓存。
